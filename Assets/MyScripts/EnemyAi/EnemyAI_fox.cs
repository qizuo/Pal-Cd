...
public class EnemyAI_fox : MonoBehaviour 
{
	...
	void Update() 
	{
		...
		case 0:
			moveSpeed =0; 
			GetComponent<Animation>().CrossFade("fox_idle");	
			break;
		case 1:
			moveSpeed = 0.3f;
			GetComponent<Animation>().CrossFade("fox_walk");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 2:
			moveSpeed = 6;
			GetComponent<Animation>()["fox_run"].layer = 2;
			GetComponent<Animation>().CrossFade("fox_run");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 3:
			moveSpeed = 0;
			GetComponent<Animation>()["fox_attack1"].layer = 3;
			GetComponent<Animation>().CrossFade("fox_attack1");
			break;
		case 4:
			moveSpeed = 0;
			GetComponent<Animation>()["fox_attack2"].layer = 3;
			GetComponent<Animation>().CrossFade("fox_attack2");
			break;
		}
