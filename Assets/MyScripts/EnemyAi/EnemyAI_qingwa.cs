...
public class EnemyAI_qingwa : MonoBehaviour 
{
	...
	void Update() 
	{
		...
		case 0:
			moveSpeed =0; 
			qingwa.GetComponent<Animation>().CrossFade("qingwa_idle");	
			break;
		case 1:
			moveSpeed = 0.3f;
			qingwa.GetComponent<Animation>().CrossFade("qingwa_run");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 2:
			moveSpeed = 6;
			qingwa.GetComponent<Animation>()["qingwa_run"].layer = 2;
			qingwa.GetComponent<Animation>().CrossFade("qingwa_run");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 3:
			moveSpeed = 0;
			qingwa.GetComponent<Animation>()["qingwa_attack1"].layer = 3;
			qingwa.GetComponent<Animation>().CrossFade("qingwa_attack1");
			break;
		case 4:
			moveSpeed = 0;
			qingwa.GetComponent<Animation>()["qingwa_attack2"].layer = 3;
			qingwa.GetComponent<Animation>().CrossFade("qingwa_attack2");
			break;
		case 5:
			moveSpeed = 0;
			qingwa.GetComponent<Animation>()["qingwa_dead"].layer = 6;
			qingwa.GetComponent<Animation>().CrossFade("qingwa_dead");
			break;
		}
