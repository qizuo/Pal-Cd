...
public class EnemyAI_tiger : MonoBehaviour 
{
	...
	void Update() 
	{
		...
		case 0:
			moveSpeed =0; 
			GetComponent<Animation>().CrossFade("Tiger_idle2");	
			break;
		case 1:
			moveSpeed = 0.6f;  
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			GetComponent<Animation>().CrossFade("Tiger_walk");		
			break;
		case 2:
			moveSpeed = 6;
			gameObject.GetComponent<Animation>()["Tiger_run"].layer = 2;
			GetComponent<Animation>().CrossFade("Tiger_run");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 3:
			moveSpeed = 0;
			gameObject.GetComponent<Animation>()["Tiger_attack1"].layer = 3;
			GetComponent<Animation>().CrossFade("Tiger_attack1");
			break;
		case 4:
			moveSpeed = 0;
			gameObject.GetComponent<Animation>()["Tiger_attack2"].layer = 3;
			GetComponent<Animation>().CrossFade("Tiger_attack2");
			break;
		case 5:
			moveSpeed = 0;
			gameObject.GetComponent<Animation>()["Tiger_dead"].layer = 6;
			GetComponent<Animation>().CrossFade("Tiger_dead");
			break;
		case 6:
			moveSpeed = 0;
			GetComponent<Animation>().CrossFade("Tiger_idle1");	
			break;
		}
	...
	void OnGUI()
	{
		...
		if(curHealth != 0)
		{
			if(curHealth < maxHealth)
			{
				...
				GUI.Label(new Rect(Screen.width/2 + 200,40,160,160),"�ϻ�","yellow1");
				GUI.Label(new Rect(Screen.width/2 + 100,130,100,20),"Lv.10","yellow");
				GUI.Label(new Rect(Screen.width/2 + 10,10,160,160),"","�ϻ�");
				GUI.Box(new Rect(Screen.width/2 + 125,70,EnemyHealthbar,20),"","Ѫ��");
