...
public class EnemyAI_wolf : MonoBehaviour 
{
	...
	void Update() 
	{
		...
		case 0:
			moveSpeed =0; 
			GetComponent<Animation>().CrossFade("wolf_idle");	
			break;
		case 1:
			moveSpeed = 0.3f;
			GetComponent<Animation>().CrossFade("wolf_walk");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 2:
			moveSpeed = 6;
			GetComponent<Animation>()["wolf_run"].layer = 2;
			GetComponent<Animation>().CrossFade("wolf_run");
			gameObject.transform.Translate(Vector3.forward*Time.deltaTime*moveSpeed);	
			break;
		case 3:
			moveSpeed = 0;
			GetComponent<Animation>()["wolf_attack"].layer = 3;
			GetComponent<Animation>().CrossFade("wolf_attack");
			break;
		}
	...
	void OnGUI()
	{
		...
		if(curHealth != 0)
		{
			if(curHealth < maxHealth)
			{
				...
				GUI.Label(new Rect(Screen.width/2 + 200,40,160,160),"Ұ��","yellow1");
				GUI.Label(new Rect(Screen.width/2 + 100,130,100,20),"Lv.7","yellow");
				GUI.Label(new Rect(Screen.width/2 + 10,10,160,160),"","Ұ��");
